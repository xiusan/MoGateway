var ConstructionInfoList = [];
var operatorInfoData = {};
var bootstrapTableOption = {
    url: 'getConstructionInfoList',
    ...
    columns: [
        ...
        {title: '序号', align: 'center', formatter: 'increases'},
            { field: 'construction', title: '建设场所ID'},
            { field: 'constructionName', title: '建设场所名称'},
            { field: 'createUser', title: '创建者'},
            { field: 'createDttm', title: '创建时间'},
            { field: 'updateUser', title: '更新者'},
            { field: 'updateDttm', title: '更新时间'},
...
$(document).ready(function() {
    ...
    setBootstrapTable('#operatorConstructionInfo', bootstrapTableOption);  //初始化页面table
});

function getChargingStationName(id) {
    changeSelectpicker(id, 'destroy');
    $(id).empty();
    if (ConstructionInfoList && ConstructionInfoList.length > 0) {
        setChargingStationName(id, ConstructionInfoList);
    } else {
